...
var getDataField = function(){
    ...
    if(selectedToken && selectedToken !== 'tlot') {
...
Template['views_send'].onCreated(function(){
    ...
    if(FlowRouter.getRouteName() === 'deployContract') {
        ...
        TemplateVar.set('selectedToken', 'tlot');

    // Send funds
    } else {
        TemplateVar.set('selectedAction', 'send-funds');
        TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'tlot');
    }
    ...
    template.autorun(function(c){
        ...
        if(!c.firstRun && TemplateVar.get('selectedToken') === 'tlot') {
...
Template['views_send'].helpers({
    ...
    */
    'total': function(ether){
        ...
        if (TemplateVar.get('selectedToken') === 'tlot') {
    ...
    */
    'submit form': function(e, template){
        ...
        if(selectedAccount && !TemplateVar.get('sending')) {
            ...
            if(sendAll && (selectedAccount.owners || tokenAddress !== 'tlot'))
                sendAll = false;
            ...
            if(selectedAccount.balance === '0' && (!selectedAccount.owners || tokenAddress === 'tlot'))
                return GlobalNotification.warning({
